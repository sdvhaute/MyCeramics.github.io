...
const getActivityController = (req, res) => {
  ...
  pool.query(
    `SELECT * FROM projects2 WHERE thrown = false AND trimmed = false AND bisque = false AND glazed = false AND glazefired= false `,
    (err, results) => {
      if (err) throw err;
      let countNew = results.rows.length;
      pool.query(
        `SELECT * FROM projects2 WHERE thrown = true`,
        (err, results) => {
          if (err) throw err;
          let countThrown = results.rows.length;
          pool.query(
            `SELECT * FROM projects2 WHERE trimmed = true`,
            (err, results) => {
              if (err) throw err;
              let countTrimmed = results.rows.length;
              pool.query(
                `SELECT * FROM projects2 WHERE bisque = true`,
                (err, results) => {
                  if (err) throw err;
                  let countBisque = results.rows.length;
                  pool.query(
                    `SELECT * FROM projects2 WHERE glazed = true`,
                    (err, results) => {
                      if (err) throw err;
                      let countGlazed = results.rows.length;
                      pool.query(
                        `SELECT * FROM projects2 WHERE glazefired = true`,
                        (err, results) => {
                          if (err) throw err;
        ...
        if (files.upload.size !== 0) {
          cloudinary.uploader.upload(
            ...
            (result) => {
              ...
              pool.query(
                `INSERT INTO images (title, cloudinary_id, image_url, project_id) VALUES($1,$2,$3, $4) RETURNING *`,
                [
                  uploadObject.title,
                  uploadObject.cloudinary_id,
                  uploadObject.image_url,
                  projectObject[0].id,
                ],
                (err, results) => {
                  if (err) throw err;
                  ...
                  pool.query(
                    `UPDATE projects2 SET imgurl = $1 WHERE id = $2`,
                    [uploadObject.image_url, imageObject[0].project_id],
                    (err, results) => {
                      if (err) throw err;
...
const updateProjectParametersController = (req, res) => {
  ...
  for (let prop in reqObject) {
    if (reqObject[prop] === "on") {
      pool.query(
        `UPDATE projects2 SET ${prop} = true WHERE id = $1`,
        [projectid],
        (err) => {
      ...
      pool.query(
        `UPDATE projects2 SET ${prop} = false WHERE id = $1`,
        [projectid],
        (err) => {
  ...
  pool.query(
    `SELECT * FROM projects2 WHERE id=$1`,
    [projectid],
    (err, results) => {
      let projectObject = results.rows;
      res.redirect(`/api/v1/projects/${projectObject[0].id}`);
...
const checkThrown = async (req, res) => {
  ...
  if (thrown === "on") {
    pool.query(
      `UPDATE projects2 SET thrown = true, throwndate = NOW() WHERE id = $1`,
      [projectid],
      (err) => {
        if (err) throw err;
      }
    ...
    pool.query(
      `UPDATE projects2 SET thrown = false, throwndate = null WHERE id = $1`,
      [projectid],
      (err) => {
        if (err) throw err;
      }
...
const checkTrimmed = async (req, res) => {
  ...
  if (trimmed === "on") {
    pool.query(
      `UPDATE projects2 SET trimmed = true, trimmeddate = NOW() WHERE id = $1`,
      [projectid],
      (err) => {
        if (err) throw err;
      }
    ...
    pool.query(
      `UPDATE projects2 SET trimmed = false, trimmeddate = null WHERE id = $1`,
      [projectid],
      (err) => {
        if (err) throw err;
      }
...
const checkBisque = async (req, res) => {
  ...
  if (bisque === "on") {
    pool.query(
      `UPDATE projects2 SET bisque = true, bisquedate = NOW() WHERE id = $1`,
      [projectid],
      (err) => {
        if (err) throw err;
      }
    ...
    pool.query(
      `UPDATE projects2 SET bisque = false, bisquedate = null WHERE id = $1`,
      [projectid],
      (err) => {
        if (err) throw err;
      }
...
const checkGlazed = async (req, res) => {
  ...
  if (glazed === "on") {
    pool.query(
      `UPDATE projects2 SET glazed = true, glazeddate = NOW() WHERE id = $1`,
      [projectid],
      (err) => {
        if (err) throw err;
      }
    ...
    pool.query(
      `UPDATE projects2 SET glazed = false, glazeddate = null WHERE id = $1`,
      [projectid],
      (err) => {
        if (err) throw err;
      }
...
const checkGlazeFired = async (req, res) => {
  ...
  if (glazefired === "on") {
    pool.query(
      `UPDATE projects2 SET glazefired = true, glazefireddate = NOW() WHERE id = $1`,
      [projectid],
      (err) => {
        if (err) throw err;
      }
    ...
    pool.query(
      `UPDATE projects2 SET glazefired = false, glazefireddate = null WHERE id = $1`,
      [projectid],
      (err) => {
        if (err) throw err;
      }
...
const updateNotes = async (req, res) => {
  ...
  pool.query(
    `UPDATE projects2 SET notes = $2 WHERE id = $1`,
    [projectid, notes],
    (err) => {
      if (err) throw err;
    }
...
const searchStringController = (req, res) => {
  ...
  pool.query(
    `SELECT * FROM projects2 WHERE lower(projectname) LIKE lower('%${stringSearch}%') AND user_id=${user_id}`,
    (err, results) => {
      if (err) throw err;
...
const deleteProjectController = async (req, res) => {
  ...
  pool.query(queries.deleteProject, [projectid], (err, results) => {
    ...
    req.flash(
      "error",
      `Succesfully deleted project: ${deletedObject[0].projectname}`
    );
